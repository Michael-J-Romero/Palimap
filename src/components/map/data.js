...
for (let i = 0; i < tempData1.length; i++) {
  const { coords } = tempData1[i];

  const data = await loadParcel({ latlng: coords });
  const { SitusAddress, SitusStreet, SitusCity, SitusZIP, LegalDescLine1, SitusFullAddress } = data.properties || {};
  ...
  if (!SitusStreet
    ...
    || SitusStreet === "0"  
    || SitusStreet === "null"
    || SitusStreet === "undefined"
    || SitusStreet === "N/A"
    || SitusStreet === "NaN"
    

  ){
    console.warn("No SitusAddressaaa23 found for APN:", data.properties);
    const apnData = await loadParcel({ apn: apns[c] });

    tempData1[i].apn = apnData.properties.APN;
    tempData1[i].coords = {
      lat: apnData.properties.CENTER_LAT,
      lng: apnData.properties.CENTER_LON,
    };
    tempData1[i].geometry = apnData.geometry;

    c++;
  } else {
    // tempData1[i].apn = data.properties.APN;
    // tempData1[i].coords = {
    //   lat: data.properties.CENTER_LAT,
    //   lng: data.properties.CENTER_LON,
    // };
